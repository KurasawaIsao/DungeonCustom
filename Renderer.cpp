...
ID3D11VertexShader* Renderer::s_VS = nullptr;
ID3D11PixelShader* Renderer::s_PS = nullptr;
ID3D11InputLayout* Renderer::s_Layout = nullptr;
...
{
	XMMATRIX transposed[256];
	for (UINT i = 0; i < count && i < 256; i++)
	{
		transposed[i] = XMMatrixTranspose(matrices[i]); // GPU送信用に転置
	}
	m_DeviceContext->UpdateSubresource(m_BoneBuffer, 0, NULL, transposed, 0, 0);
...
void Renderer::SetLight(LIGHT Light)
{
	...
	m_DeviceContext->VSSetConstantBuffers(4, 1, &m_LightBuffer); // 頂点シェーダー用
	m_DeviceContext->PSSetConstantBuffers(4, 1, &m_LightBuffer); // ピクセルシェーダー用
}
...
{
	CreateVertexShader(&s_VS, &s_Layout,
		"shader\\pixelLightingBlinnPhongVS.cso");
	CreatePixelShader(&s_PS,
